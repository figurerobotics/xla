...
def _update_options(nvcc_options):
    ...
    return [update_options[opt] if opt in update_options else opt for opt in nvcc_options]
...
def InvokeNvcc(argv, log=False):
    ...
    fatbin_options = "".join(
        [" --fatbin-options=" + option for option in GetOptionValue(argv, "-Xcuda-fatbinary")]
    )
    ...
    # XXX: This has caused more trouble than it seems worth.
    # We should probably just pass the list of source files as is to nvcc.
    src_files = [f for f in src_files if re.search(r"\.(cpp|cc|c|cxx|C|cu)'?$", f)]
    srcs = " ".join(src_files)
    out = " -o " + out_file[0]
    ...
    if depfiles:
        ...
        cmd = (
            ...
            + nvccopts
            + ' --compiler-options "'
            # XXX: Passing the sysroot like this is also very ugly
            + "--sysroot=external/_main~sysroots_ext~amd64_sysroot "
